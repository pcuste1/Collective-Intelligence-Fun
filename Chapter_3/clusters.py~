...
def readfile(filename):
    ...
    for line in lines[1:]:
        ...
        # Dealing with rows that lack full data
        m = [float(x) for x in p[1:]]
        while len(m) < len(colnames):
            m.append(0.0)
        ...
        for i in range(len(clust)):
            for j in range(i+1, len(clust)):
                ...
                if d < closest:
                    closest = d
                    lowestpair = (i, j)
        # Calculate the average of the to clusters
        mergevec = [
...
def printclust(clust, labels=None, n=0):
    # indent to make a heirachy layout 
    for i in range(n): 
        print ' ',
    if clust.id<0:
        ...
        # positive idmeans that this is an endpoint
...
def drawnode(draw, clust, x, y, scaling, labels):
    if clust.id < 0:
        h1 = getheight(clust.left)*10
        h2 = getheight(clust.right)*10
        top = y - (h1+h2)/2
        bottom = y + (h1+h2)/2
...
def kcluster(rows, distance=pearson, k=4):
    ...
    for i in range(len(rows[0])):
        ranges.append((min([row[i] for row in rows]), max([row[i] for row in rows])))
    ...
    for t in range(100):
        ...
        # If the results are the same as last time, the is complete
